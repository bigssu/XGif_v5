...
def _is_hdr_active_win32() -> bool:
    ...
    try:
        ...
        path_size = num_path.value * 72  # DISPLAYCONFIG_PATH_INFO ~72 bytes per path
        mode_size = num_mode.value * 64  # DISPLAYCONFIG_MODE_INFO ~64 bytes
        path_buf = (ctypes.c_byte * path_size)()
        mode_buf = (ctypes.c_byte * mode_size)()
        ...
        # DISPLAYCONFIG_PATH_INFO: sourceInfo(20), targetInfo(20), flags(4)
        # targetInfo: adapterId(LUID 8), id(4), ...
        adapter_low = int.from_bytes(path_buf[20:24], "little")
        adapter_high = int.from_bytes(path_buf[24:28], "little", signed=True)
        target_id = int.from_bytes(path_buf[28:32], "little")
