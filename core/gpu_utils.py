...
def _check_cupy():
    ...
    with _cupy_check_lock:
        ...
        try:
            ...
            logger.info("CuPy not available: %s", e)
        except (RuntimeError, OSError, AttributeError) as e:
            logger.info("CuPy initialization failed: %s", e)
        except Exception as e:
            logger.info("CuPy unexpected error: %s", e)
...
def detect_gpu(skip_cupy: bool = False) -> GpuInfo:
    ...
    # 하드웨어 감지 (느린 작업은 락 외부에서 실행)
    if not info.has_cuda and not info.gpu_name:
        gpu_detected = False
    ...
    # CuPy 확인 (락 외부 — 초기화가 오래 걸릴 수 있음)
    if not skip_cupy and info.has_cuda and not info.has_cupy:
        try:
            cupy_ok = _check_cupy()
