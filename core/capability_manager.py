...
class CapabilityManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        """
        Args:
            cache_dir: 캐시 파일 저장 디렉토리 (None이면 %APPDATA%/GifRecoder2)
        """
        if cache_dir:
            self._cache_dir = Path(cache_dir)
        else:
            appdata = os.environ.get('APPDATA', os.path.expanduser('~'))
            self._cache_dir = Path(appdata) / 'GifRecoder2'
    ...
    def _select_optimal_pipeline(self, caps: SystemCapabilities) -> Pipeline:
        ...
        if caps.best_h265_encoder:
            ...
            if 'nvenc' in encoder:
                ...
                name = "Pipeline A: DXGI + NVENC H.265" if capture_backend == "dxcam" else "Pipeline B: GDI + NVENC H.265"
            elif 'qsv' in encoder:
        ...
        if caps.best_h264_encoder:
            ...
            if 'nvenc' in encoder:
                score = capture_score + 45
                name = f"Pipeline: {capture_backend.upper()} + NVENC H.264"
            elif 'qsv' in encoder:
                score = capture_score + 35
