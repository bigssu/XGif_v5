    ...
    def _on_direct_install(self, event):
        ...
        def do_install():
            try:
                ...
                wx.CallAfter(_on_done, False, str(e))
...
def _install_ffmpeg(parent, dep_status):
    """FFmpeg 설치 흐름 — FFmpegDownloader + ProgressDialog

    다운로드/압축 해제 진행률을 ProgressDialog에 실시간 반영.
    백그라운드 스레드에서 wx.CallAfter로 전달되는 콜백을
    메인 스레드에서 안전하게 처리한다.
    """
    ...
    # 공유 상태 — 백그라운드 스레드 → 메인 스레드
    state = {'success': False, 'message': '', 'done': False,
             'phase': 'download'}  # download | extract
    ...
    def on_progress(downloaded, total):
        if total > 0:
            ...
            if state['phase'] == 'download':
                ...
                ext_mb = downloaded / (1024 * 1024)
                total_mb = total / (1024 * 1024)
                msg = f"압축 해제 중...\n{ext_mb:.1f} MB / {total_mb:.1f} MB"
            try:
                keep_going, _ = progress.Update(min(percent, 99), msg)
                if not keep_going:
                    downloader.cancel()
            except Exception:
                pass
    ...
    # 완료 대기 — 이벤트 루프를 유지하되, 스레드 종료까지 안전하게 대기
    # 재진입 방지를 위해 부모 윈도우 비활성화
    if parent:
        parent.Disable()
    while not state['done']:
        wx.MilliSleep(50)
        wx.GetApp().Yield(True)
        # 스레드가 이미 종료됐는데 done이 아직 False인 경우 (콜백 유실) 대비
        if not downloader.is_alive() and not state['done']:
            # 스레드 종료 후 마지막 pending 이벤트 한 번 더 처리
            wx.MilliSleep(200)
            wx.GetApp().Yield(True)
            if not state['done']:
                # 콜백이 끝내 도착하지 않음 — 수동 판정
                from core.ffmpeg_installer import is_ffmpeg_installed
                state['success'] = is_ffmpeg_installed()
                state['message'] = "FFmpeg 설치 완료" if state['success'] else "FFmpeg 설치 실패"
                state['done'] = True
