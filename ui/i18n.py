...
class TranslationManager:
    ...
    def __init__(self):
        ...
        self._main_translations_ko = {
            ...
            'dep_use_cpu_instead': 'CPU 모드로 계속',
            ...
            # CuPy 설치 가이드 다이얼로그
            'cupy_guide_title': 'GPU 가속 설정',
            'cupy_guide_msg': 'NVIDIA GPU가 감지되었지만 CuPy가 설치되어 있지 않습니다.\n\nGPU: {gpu_name}\nCUDA 드라이버: {cuda_version}\n\nGPU 가속을 사용하려면 아래 명령어로 CuPy를 설치하세요.',
            'cupy_guide_cmd': 'pip install {package}',
            'cupy_guide_copy': '명령어 복사',
            'cupy_guide_copied': '복사됨!',
            'cupy_guide_recheck': '재검사',
            'cupy_guide_frozen_msg': 'GPU 가속을 사용하려면 CuPy가 필요합니다.\n시스템에 Python 3.11 이상이 설치되어 있어야 합니다.\n\n아래 명령어를 터미널(CMD/PowerShell)에서 실행하세요.\n설치 후 앱을 재시작하면 GPU 가속이 활성화됩니다.',
            'cupy_guide_dev_msg': 'GPU 가속을 사용하려면 CuPy가 필요합니다.\n\n아래 명령어를 실행하거나, "직접 설치" 버튼을 눌러 자동 설치할 수 있습니다.',
            'cupy_guide_direct_install': '직접 설치',
            'cupy_cpu_mode_notice': 'CuPy가 설치되지 않아 CPU 모드로 실행됩니다.\nGPU 가속을 사용하려면 CuPy를 설치하세요.',
            'cupy_guide_no_nvidia': 'NVIDIA GPU가 필요합니다.\n\nGPU 가속은 NVIDIA GPU와 CUDA 드라이버가 필요합니다.\n현재 시스템에서 NVIDIA GPU가 감지되지 않았습니다.',
            ...
            # 필수 의존성 다이얼로그
            'dep_required_title': '필수 환경 설정',
            'dep_required_desc': '이 앱을 사용하려면 아래 항목이 필요합니다.',
            'dep_python_desc': '패키지 설치에 필수 (Python 3.11 이상)',
            'dep_python_download': '다운로드',
            'dep_python_download_url': 'https://www.python.org/downloads/release/python-3119/',
            'dep_recheck_btn': '재검사',
            'dep_continue_btn': '계속',
            'dep_exit_btn': '종료',
            'dep_ffmpeg_required_desc': '녹화 및 인코딩에 필수',

            # StartupCheckDialog (통합 환경 진단)
            'scd_title': '환경 진단',
            'scd_desc': '앱 실행에 필요한 환경을 확인합니다. 누락된 항목은 자동으로 설치할 수 있습니다.',
            'scd_col_dep': '항목',
            'scd_col_required': '필수 여부',
            'scd_col_status': '상태',
            'scd_col_details': '설명',
            'scd_required': '필수',
            'scd_optional': '선택',
            'scd_status_unchecked': '확인 대기',
            'scd_status_checking': '확인 중…',
            'scd_status_pass': '설치됨',
            'scd_status_missing': '미설치',
            'scd_status_missing_required': '필수 (설치 필요)',
            'scd_status_missing_optional': '선택 (미설치)',
            'scd_status_skipped': '건너뜀',
            'scd_status_installing': '설치 중…',
            'scd_status_install_ok': '설치 완료',
            'scd_status_install_fail': '설치 실패',
            'scd_status_error': '오류',
            'scd_show_log': '▶ 로그 보기',
            'scd_hide_log': '▼ 로그 숨기기',
            'scd_btn_recheck': '다시 검사',
            'scd_btn_retry_install': '설치 재시도',
            'scd_btn_install_missing': '누락 항목 설치',
            'scd_btn_skip': '건너뛰기',
            'scd_btn_continue': '계속',
            'scd_missing_required_warn': '다음 필수 항목이 설치되지 않았습니다:\n{names}\n\n앱이 정상적으로 동작하지 않을 수 있습니다.\n그래도 건너뛰시겠습니까?',
            'scd_busy_wait': '작업이 진행 중입니다. 완료 후 다시 시도하세요.',
            'scd_skipped_by_user': '사용자가 건너뜀',
            ...
            'gpu_status_tooltip': '클릭하여 GPU 정보 확인',
            'gpu_initializing': 'GPU 확인 중...',
            'gpu_info_title': 'GPU 정보',
            'gpu_info_msg': '{name}\n메모리: {memory}MB\nCuPy: {cupy}\nNVENC: {nvenc}\n드라이버: {driver}',
            'gpu_not_found_msg': 'NVIDIA GPU를 찾을 수 없습니다',
        ...
        self._main_translations_en = {
            ...
            'dep_skip_btn': 'Continue in CPU mode',
            'dep_close_btn': 'Close',
            ...
            'dep_use_cpu_instead': 'Continue in CPU mode',
            ...
            # CuPy install guide dialog
            'cupy_guide_title': 'GPU Acceleration Setup',
            'cupy_guide_msg': 'NVIDIA GPU detected but CuPy is not installed.\n\nGPU: {gpu_name}\nCUDA Driver: {cuda_version}\n\nInstall CuPy with the command below to enable GPU acceleration.',
            'cupy_guide_cmd': 'pip install {package}',
            'cupy_guide_copy': 'Copy Command',
            'cupy_guide_copied': 'Copied!',
            'cupy_guide_recheck': 'Recheck',
            'cupy_guide_frozen_msg': 'CuPy is required for GPU acceleration.\nPython 3.11+ must be installed on your system.\n\nRun the command below in a terminal (CMD/PowerShell).\nRestart the app after installation to enable GPU acceleration.',
            'cupy_guide_dev_msg': 'CuPy is required for GPU acceleration.\n\nRun the command below, or click "Install Now" for automatic installation.',
            'cupy_guide_direct_install': 'Install Now',
            'cupy_cpu_mode_notice': 'CuPy is not installed. Running in CPU mode.\nInstall CuPy to enable GPU acceleration.',
            'cupy_guide_no_nvidia': 'NVIDIA GPU is required.\n\nGPU acceleration requires an NVIDIA GPU with CUDA drivers.\nNo NVIDIA GPU was detected on this system.',
            ...
            # Required dependency dialog
            'dep_required_title': 'Required Setup',
            'dep_required_desc': 'The following items are required to use this app.',
            'dep_python_desc': 'Required for package installation (Python 3.11+)',
            'dep_python_download': 'Download',
            'dep_python_download_url': 'https://www.python.org/downloads/release/python-3119/',
            'dep_recheck_btn': 'Recheck',
            'dep_continue_btn': 'Continue',
            'dep_exit_btn': 'Exit',
            'dep_ffmpeg_required_desc': 'Required for recording and encoding',

            # StartupCheckDialog (unified environment check)
            'scd_title': 'Environment Check',
            'scd_desc': 'Checking the environment required to run the app. Missing items can be installed automatically.',
            'scd_col_dep': 'Item',
            'scd_col_required': 'Required',
            'scd_col_status': 'Status',
            'scd_col_details': 'Description',
            'scd_required': 'Required',
            'scd_optional': 'Optional',
            'scd_status_unchecked': 'Pending',
            'scd_status_checking': 'Checking...',
            'scd_status_pass': 'Installed',
            'scd_status_missing': 'Missing',
            'scd_status_missing_required': 'Required (install needed)',
            'scd_status_missing_optional': 'Optional (not installed)',
            'scd_status_skipped': 'Skipped',
            'scd_status_installing': 'Installing...',
            'scd_status_install_ok': 'Install OK',
            'scd_status_install_fail': 'Install Failed',
            'scd_status_error': 'Error',
            'scd_show_log': '▶ Show Log',
            'scd_hide_log': '▼ Hide Log',
            'scd_btn_recheck': 'Recheck',
            'scd_btn_retry_install': 'Retry Install',
            'scd_btn_install_missing': 'Install Missing',
            'scd_btn_skip': 'Skip',
            'scd_btn_continue': 'Continue',
            'scd_missing_required_warn': 'The following required items are not installed:\n{names}\n\nThe app may not work correctly.\nDo you still want to skip?',
            'scd_busy_wait': 'Operation in progress. Please wait until it completes.',
            'scd_skipped_by_user': 'Skipped by user',
            ...
            'gpu_status_tooltip': 'Click to check GPU info',
            'gpu_initializing': 'Checking GPU...',
            'gpu_info_title': 'GPU Info',
            'gpu_info_msg': '{name}\nMemory: {memory}MB\nCuPy: {cupy}\nNVENC: {nvenc}\nDriver: {driver}',
            'gpu_not_found_msg': 'No NVIDIA GPU found',
