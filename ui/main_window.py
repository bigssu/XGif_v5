...
class MainWindow(wx.Frame):
    ...
    def _connect_capture_control_bar(self):
        ...
        self.capture_control_bar.set_help_requested_callback(self._open_help)
    ...
    def _open_help(self):
        ...
        # 이미 열려 있으면 포커스만 이동
        if getattr(self, '_help_dialog', None) and self._help_dialog:
            try:
        ...
        dlg = HelpDialog(self)
        dlg.Bind(wx.EVT_CLOSE, self._on_help_closed)
        dlg.Show()
        self._help_dialog = dlg

    def _on_help_closed(self, event):
        """도움말 다이얼로그 닫힘 처리"""
        self._help_dialog = None
        event.Skip()
    ...
    def OnClose(self, event):
        ...
        # 메인/에디터가 모두 닫힌 상태라면 잔여 윈도우 정리 후 앱 루프 종료
        try:
