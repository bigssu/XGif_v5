...
APP_SUMMARY = (
    "XGif는 Windows 화면을 GIF 또는 MP4로 녹화하고, 녹화 결과를 프레임 단위로 "
    "다듬어 저장하는 경량 캡처/편집 앱입니다."
)

EDITOR_SUMMARY = (
    "XGif Editor는 녹화된 GIF를 프레임, 시간, 크기, 효과, 텍스트, 도형, 펜슬 "
    "오버레이 단위로 빠르게 수정하는 내장 편집 화면입니다."
)
...
class HelpDialog(ThemedDialog):
    ...
    def _setup_ui(self):
        ...
        subtitle = wx.StaticText(header, label=EDITOR_SUMMARY)
        ...
        notebook.AddPage(self._create_overview_page(notebook), "요약")
        notebook.AddPage(self._create_tools_page(notebook), "편집 기능")
        notebook.AddPage(self._create_shortcuts_page(notebook), "단축키")
        notebook.AddPage(self._create_info_page(notebook), "정보")
    ...
    def _create_overview_page(self, parent):
        ...
        self._add_section(
            ...
            [
                APP_SUMMARY,
                "레코더는 캡처 영역, FPS, 해상도, GIF/MP4 출력, GPU 사용 여부를 빠르게 선택하도록 설계되었습니다.",
                "에디터는 녹화 직후 프레임 삭제, 시간 조절, 텍스트/스티커/펜슬 추가, 크롭/리사이즈/필터 적용, 저장 최적화를 처리합니다.",
            ],
        )
        self._add_section(
            page,
            "권장 흐름",
            [
                "1. 메인 화면에서 캡처 영역과 출력 형식을 정합니다.",
                "2. 녹화 후 편집 버튼으로 에디터를 열어 불필요한 프레임과 시간을 정리합니다.",
                "3. 필요한 오버레이나 효과를 적용한 뒤 저장합니다.",
            ],
        )
        return page
    ...
    def _create_tools_page(self, parent):
        ...
        self._add_section(
            ...
            "프레임과 시간",
            [
                "프레임 목록에서 선택 프레임을 삭제하거나, 재생 시간을 직접 수정할 수 있습니다.",
                "속도 조절은 전체 프레임 또는 선택 프레임 대상으로 적용됩니다.",
                "프레임 줄이기는 중복/불필요한 프레임을 정리해 파일 크기를 줄이는 작업에 사용합니다.",
            ],
        )
        self._add_section(
            page,
            "이미지 편집",
            [
                "크롭, 리사이즈, 회전, 좌우/상하 반전, 역재생, 요요 재생을 지원합니다.",
                "필터는 밝기, 대비, 색상, 흐림 등 기본 이미지 보정 작업을 처리합니다.",
                "미리보기는 적용 전 결과를 확인하기 위한 작업 화면이며, 적용/취소로 원본 보존 여부를 결정합니다.",
            ],
        )
        self._add_section(
            page,
            "오버레이",
            [
                "텍스트 추가는 글꼴, 크기, 색상, 외곽선, 애니메이션, 깜빡임 옵션을 제공합니다.",
                "스티커/도형과 말풍선은 강조 지점을 표시할 때 사용합니다.",
                "펜슬 그리기는 프레임 위에 직접 선을 그리는 주석 도구입니다.",
            ],
        )
        return page
    ...
    def _create_info_page(self, parent):
        ...
        self._add_section(
            ...
            "설계 방향",
            [
                "Windows용 wxPython 앱 구조를 유지해 번들 크기 증가를 피했습니다.",
                "UI는 다크 테마, 통일된 아이콘, 명확한 카드 깊이, 성능에 부담이 적은 레이어 구조를 기준으로 정리했습니다.",
                "Defender 회피나 Windows 보안 상태 변경 기능은 설치/빌드 흐름에서 제거했습니다.",
            ],
        )
        return page
...
class AboutDialog(ThemedDialog):
    ...
    def _setup_ui(self):
        ...
        summary = wx.StaticText(self, label=EDITOR_SUMMARY)
